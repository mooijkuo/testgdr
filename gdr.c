...
//同步 USB 與 Flash 上的 setting.
//return value = number of USB, 0=fail
/*
 * CarMagic
 * Flash 魔術數字放在位址 0
 * USB上 魔術數字放在 0xe000000
...
int GenerateUSBDiskFile(DUAL_USB_T *DualUSB)
{
	...
	/* ivan note
	 *  由此段可以看出，原系統有將 flash 畫出 mtdblock4 供儲存 USB setting 之用
	*/
	...
	/* ivan note
	 *  由此段可以看出，原系統有將 flash 畫出 mtdblock5 做為放置 Magic 之用
	*/
	// Magic ----------------------------------------------------------
...
/*
power On 時若偵測到下列情況，請用持續音提醒(響一秒，停一秒)並 Halt 系統。
1. 兩個 USB 隨身碟都異常。

當系統開始錄影時，請響三短音，表示系統開機完成。
當系統在錄影時，若偵測到記憶體可用空間小於或等於 10%，請響三
次音(響一秒，停一秒 三次)提醒使用者記憶體快滿了。此時系統自動轉成完全不覆蓋模式。
若使用者持續錄影導致記憶體空間已滿，則再用持續音提醒(響一秒，停一秒)並 Halt 系統。

提醒音設定只剩下超速與事件兩項 (可複選)。
選擇超速則系統發現超速時就發提醒音。
選擇事件則系統發現事件時就發提醒音。
兩者都選就是系統發現超速或事件時都會發提醒音。
提醒音為響一秒，停一秒。
*/
...
int ThreadLEDControl(void *gpio_pointer)
{
	...
	while (1)
	{
		switch (LEDStatus)
		{
			case LED_STATUS_INIT:	//Power On 時，所有LED燈都會點亮500ms，再顯示目前狀態
				...
				LEDStatus = LED_STATUS_STOP_RECORD;
				break;
			case LED_STATUS_LIVE_VIEW:	//當系統進入Live View時，黃色LED燈要變成閃爍顯示。亮一秒，熄一秒。
				cmd = LED_BLUE; LED_ON(cmd);
				sleep(1);
				cmd = LED_BLUE; LED_OFF(cmd);
				sleep(1);
				break;
			case LED_STATUS_PASSWORD_RESTORE:	 //當系統進入回復 Password 時，黃色LED燈要變成快速閃爍顯示。 亮300ms，熄300ms
				cmd = LED_BLUE; LED_ON(cmd);
				usleep(300*1000);
				cmd = LED_BLUE; LED_OFF(cmd);
				usleep(300*1000);
...
#endif
		if (counter > 14) //ivan note: 由於 > 4 就 break 了，所以這個 if 根本跑不到
		{
...
//Serial=0. 表示要修復第一個DATA_BLOCK. 並且serialIndex應該要從1開始.得到的.同時有start/stop flag
int FixFirstDataBlockAndIndex(CAR_RECORDER_T *CarRecorder)
{
	...
	//修第一個DATA Block
	//先取得.DataBlockSerial.DataBlockTimeStamp[6].DataBlockEndSerial.DataBlockEndTimeStamp[6]
	//FailIndex
	...
	//如果FailIndex=-1, 表示整個DATA Block都是好的
	//FailIndex標記. 在第幾個512Byte的Index出錯.
	VBUS_CHECK();
...
//先讀出來.以供修復.
	...
	//既然目前沒stop.那就有2種可能. 一種是剛好只有自己的DATA_BLOCK(16MB)剛好滿了.index也寫了.
	//另一種是. 下一個DATA_BLOCK填到一半. 這必須要用serial的連續性來判斷.
	//所以檢查的是DataBlock.
	//1. 一開始的DataBlockSerial要index/data相符
	//2. 再來要查DataBlockSerial連續到哪邊斷掉.
	//3. checksum必查的
	//4. 所以如果斷在自己這邊. 再自己這邊加stop.
	//5. 如果斷在下一個DATA_BLOCK. 那就加一個INDEX.
	...
	//如果FailIndex=-1, 表示整個DATA Block都是好的
	//FailIndex標記. 在第幾個512 bytes 的 Index 出錯.
	if (FailIndex == 0)
	{
		DDPRINTF("[%s] Fatal error!! index 0 fail.\n", __func__);
		return (-2);
	}
	//錯誤在第一個 block 裡，只要加上. stop . end time.
	else if ( (FailIndex<=(DATA_MACRO_BLOCK/DATA_BLOCK_SIZE)) && (FailIndex>0)) 
	{
		...
		//CarRecorder->DataBlockWriteOffset = MaxIndexPositionLong+DATA_MACRO_BLOCK;	//@@下個開始寫的地方
#if defined(AUTO_SPACE)
	...
	//只要加上. stop . end time. 並且加到下一個index
	else
	{
...
int FixIndexTable(CAR_RECORDER_T *CarRecorder)
{
	...
	//CheckIndexTable 把整個 IndexSectorTable 掃一遍，找出編號最大的
...
DDPRINTF("\033[1;35m[%s]\033[m usb %d MaxIndexTableSerial=(%08x, %08x), pos=(%d, %d)\n",
	...
	//兩份 table 都是 0 的話，可見整個 pen driver 都還未做過 initial
...
/*
所以有強制錄影時..最後一段的修復會有三個情況
1. 下一段就是最後ㄧ個block..跟目前的SerialIndex連號
2. 下一段是不連續的強制錄影..所以要跳跳跳..跳到沒強制錄影的普通錄影...
3. 下一段也是強制錄影..但是MarkTable還沒有標記..只在subTable有標記
這版先不做強制錄影啦!!
*/
	...
	if (MaxIndexTableSerial[0]==0)
 	{
		...
		FixFirstDataBlockAndIndex(CarRecorder);	//Mask->改返回值?
	}
...
int FillDataTable(CAR_RECORDER_T *CarRecorder)
{
	...
	*/
	...
	if (CarRecorder->FlagRecordStart[CarRecorder->CurrentChannel])	//開機時是1.
	{
